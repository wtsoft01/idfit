-- Keep auto-collection sources eligible even after repeated transient collector failures.
...
begin
  ...
  set metadata = next_metadata,
      status = case when ok_input and status = 'throttled'::public.source_status then 'live'::public.source_status else status end,
      updated_at = now()
  where id = source_id_input;
