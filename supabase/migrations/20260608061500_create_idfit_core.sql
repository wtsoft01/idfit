...
create table if not exists public.idfit_profiles (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null unique references auth.users(id) on delete cascade,
  full_name text not null default '',
  role public.idfit_app_role not null default 'customer',
...
as $$
  select exists (
    select 1
    from public.idfit_profiles
    where user_id = _user_id
      and role = any(_roles)
  );
$$;

create or replace function public.idfit_next_order_no()
returns text
language plpgsql
set search_path = public
as $$
...
create or replace function public.idfit_set_order_no()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.order_no is null or new.order_no = '' then
    new.order_no = public.idfit_next_order_no();
  end if;
  return new;
end;
$$;

create or replace function public.idfit_calculate_sale_price(
...
create trigger idfit_profiles_updated_at before update on public.idfit_profiles for each row execute function public.idfit_update_updated_at();
create trigger telegram_sources_updated_at before update on public.telegram_sources for each row execute function public.idfit_update_updated_at();
create trigger sellers_updated_at before update on public.sellers for each row execute function public.idfit_update_updated_at();
create trigger margin_rules_updated_at before update on public.margin_rules for each row execute function public.idfit_update_updated_at();
create trigger product_candidates_updated_at before update on public.product_candidates for each row execute function public.idfit_update_updated_at();
create trigger products_updated_at before update on public.products for each row execute function public.idfit_update_updated_at();
create trigger orders_updated_at before update on public.orders for each row execute function public.idfit_update_updated_at();
create trigger supplier_purchase_jobs_updated_at before update on public.supplier_purchase_jobs for each row execute function public.idfit_update_updated_at();
create trigger delivery_items_updated_at before update on public.delivery_items for each row execute function public.idfit_update_updated_at();
create trigger as_tickets_updated_at before update on public.as_tickets for each row execute function public.idfit_update_updated_at();
create trigger orders_set_order_no before insert on public.orders for each row execute function public.idfit_set_order_no();

alter table public.idfit_profiles enable row level security;
...
create policy "profiles_select_own_or_staff" on public.idfit_profiles for select to authenticated using (user_id = auth.uid() or public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));
create policy "profiles_update_own" on public.idfit_profiles for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy "staff_manage_sources" on public.telegram_sources for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));
create policy "staff_manage_sellers" on public.sellers for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));
create policy "staff_manage_raw_messages" on public.raw_messages for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));
create policy "staff_manage_margin_rules" on public.margin_rules for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin']::public.idfit_app_role[]));
create policy "staff_manage_candidates" on public.product_candidates for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));
...
create policy "staff_manage_products" on public.products for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));

create policy "customers_select_own_orders" on public.orders for select to authenticated using (user_id = auth.uid() or public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));
create policy "customers_insert_own_orders" on public.orders for insert to authenticated with check (user_id = auth.uid());
create policy "staff_update_orders" on public.orders for update to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));

create policy "staff_manage_purchase_jobs" on public.supplier_purchase_jobs for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator']::public.idfit_app_role[]));

create policy "customers_select_visible_delivery" on public.delivery_items for select to authenticated using (exists (select 1 from public.orders o where o.id = order_id and o.user_id = auth.uid() and visible_to_customer = true) or public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));
create policy "staff_manage_delivery" on public.delivery_items for all to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));

create policy "customers_manage_own_as_tickets" on public.as_tickets for select to authenticated using (user_id = auth.uid() or public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));
...
create policy "staff_update_as_tickets" on public.as_tickets for update to authenticated using (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[])) with check (public.idfit_has_role(auth.uid(), array['owner','admin','operator','support']::public.idfit_app_role[]));

create index if not exists idx_idfit_profiles_user_id on public.idfit_profiles(user_id);
